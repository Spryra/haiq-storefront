...
export default function BuildYourBoxPage() {
  ...
  const [selections, setSelections] = useState({}) // { productId: count }
  const [isSpecialDay, setIsSpecialDay] = useState(false)

  const totalSelected = Object.values(selections).reduce((s, n) => s + n, 0)
  const remaining     = BOX_SIZE - totalSelected
  const isFull        = totalSelected === BOX_SIZE
  const pct           = Math.round((totalSelected / BOX_SIZE) * 100)

  // Box price — always a single value shown
  const BOX_PRICE = isSpecialDay ? 40000 : 80000

  useEffect(() => {
    // Check if today is a special day (determines box price)
    api.get('/special-days/active-today')
      .then(res => setIsSpecialDay(res.data.isSpecialDay === true))
      .catch(() => {})
  }, [])
  ...
  const add    = (productId) => { if (!isFull) setSelections(s => ({ ...s, [productId]: (s[productId] || 0) + 1 })) }
  const remove = (productId) => {
    setSelections(s => {
      const next = { ...s, [productId]: Math.max(0, (s[productId] || 0) - 1) }
  ...
  const addBoxToCart = () => {
    // Build selections array for addBox
  ...
  return (
    <div style={{ background: '#0E0600', minHeight: '100vh' }}>
      ...
      {/* Header */}
      <div className="border-b py-16 md:py-24 px-6 md:px-16" style={{ borderColor: 'rgba(184,117,42,0.2)' }}>
        ...
        <h1
          ...
          style={{ fontSize: 'clamp(2.6rem, 6.5vw, 5.5rem)', color: '#F2EAD8' }}
        >
          Made For You.
        </h1>
        ...
        <p className="text-base max-w-sm leading-relaxed" style={{ color: 'rgba(242,234,216,0.45)' }}>
          Pick exactly 4 cookies. Any combination you want.
        </p>
        ...
        <div className="mt-5 inline-flex items-center gap-3 px-4 py-2.5"
          ...
          <p className="text-[10px] uppercase tracking-[0.25em] font-semibold" style={{ color: '#8C7355' }}>
            Box Price
          </p>
          ...
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-4">
              <span className={`font-bold text-sm`}
                style={{ color: isFull ? '#E8C88A' : '#F2EAD8' }}>
                {totalSelected}
                <span className="font-normal" style={{ color: 'rgba(242,234,216,0.3)'}}> / {BOX_SIZE}</span>
              </span>
              {!isFull && (
                <span className="text-xs" style={{ color: 'rgba(242,234,216,0.35)' }}>
                  {remaining} more {remaining === 1 ? 'cookie' : 'cookies'} to go
                </span>
              )}
              {isFull && (
                <span className="text-xs font-semibold" style={{ color: '#E8C88A' }}>
                  Box complete ✓
                </span>
              )}
            </div>
            {isFull && (
              <button
                ...
                className="px-5 py-2 font-bold text-[11px] tracking-[0.2em] uppercase transition hover:opacity-90"
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {products.map(p => {
              ...
              const soldOut = (p.variants?.find(v => v.is_default)?.stock_qty ?? 1) === 0
              ...
              return (
                ...
                >
                  ...
                  <div className="overflow-hidden" style={{ aspectRatio: '1' }}>
                    <img
                      src={LOCAL_IMGS[p.slug] || '/HAIQmain.png'}
                      alt={p.name}
                      loading="lazy"
                  ...
                  {count > 0 && (
                    <div
                      className="absolute top-2 right-2 w-7 h-7 rounded-full font-bold text-sm flex items-center justify-center shadow-lg"
                      style={{ background: '#B8752A', color: '#1A0A00' }}
                    >
                      {count}
                    </div>
                  ...
                  {/* Info + controls */}
                  <div className="p-3">
                    <p className="font-serif font-bold text-sm leading-tight mb-0.5" style={{ color: '#F2EAD8' }}>
                      {p.name}
                    </p>
                    <p className="text-[10px] mb-3 line-clamp-1" style={{ color: '#8C7355' }}>
                      {p.subtitle}
                    </p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="w-8 h-8 flex items-center justify-center text-lg transition disabled:opacity-20"
                        style={{ background: 'rgba(184,117,42,0.2)', border: '1px solid rgba(184,117,42,0.5)', color: '#B8752A' }}
                        onMouseEnter={e => { if (!isFull) { e.currentTarget.style.background = '#B8752A'; e.currentTarget.style.color = '#1A0A00' }}}
                        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(184,117,42,0.2)'; e.currentTarget.style.color = '#B8752A' }}
                      >+</button>
                    </div>
                  </div>
                </div>
      ...
      {isFull && (
        <div
          className="fixed bottom-0 left-0 right-0 z-40 py-4 px-6"
        ...
        >
          <div className="container mx-auto max-w-2xl flex items-center justify-between gap-4">
            <div>
              <p className="font-bold text-sm" style={{ color: '#F2EAD8' }}>Your box is ready.</p>
              <p className="text-xs mt-0.5" style={{ color: '#8C7355' }}>
                {Object.entries(selections)
                  .map(([id, count]) => { const p = products.find(pr => pr.id === id); return p ? `${count}× ${p.name}` : '' })
                  .filter(Boolean).join(' · ')}
              </p>
            </div>
            <button
              onClick={addBoxToCart}
              className="px-8 py-3 font-bold text-[11px] tracking-[0.25em] uppercase whitespace-nowrap transition hover:opacity-90"
              style={{ background: '#B8752A', color: '#1A0A00' }}
            >
