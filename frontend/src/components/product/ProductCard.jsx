// ProductCard.jsx — fallback image now uses the HAIQ main logo, not placeholder-product.webp
...
export default function ProductCard({ product, index = 0 }) {
  ...
  const handleAdd = (e) => {
    ...
    e.stopPropagation()
    if (isSoldOut) return
    if (hasMultiple) { setShowPicker(true); return }
          ...
          <div className="flex flex-col h-full bg-light rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300">
            ...
            <div className="flex flex-col flex-1 p-4">
              ...
              <button
                ...
                `}
              >
                {isSoldOut ? 'Sold Out' : added ? '✓ Added' : hasMultiple ? 'Choose Size →' : 'Add to Cart'}
              </button>
            </div>
          </div>
        </Link>
