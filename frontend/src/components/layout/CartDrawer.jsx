...
function BoxCartItem({ item, onRemove }) {
  ...
  return (
    <div className="py-4" style={{ borderBottom: '1px solid rgba(166,124,82,0.12)' }}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="font-serif font-bold text-sm leading-snug" style={{ color: '#F5EAD8' }}>
                {item.name}
              </p>
          ...
          >
            <div className="mt-2 space-y-1 pl-0.5" style={{ borderLeft: '2px solid rgba(166,124,82,0.25)', paddingLeft: '10px', paddingTop: '4px' }}>
              {item.boxContents?.map((cookie, i) => (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-[10px]" style={{ color: '#8C7355' }}>
                    {cookie.quantity}×
                  </span>
                  <span className="text-[10px]" style={{ color: 'rgba(245,234,216,0.55)' }}>
                    {cookie.name}
                  </span>
                </div>
              ))}
...
function SingleCartItem({ item, onRemove, onQtyChange }) {
  ...
  return (
    <div className="flex gap-3 py-4" style={{ borderBottom: '1px solid rgba(166,124,82,0.12)' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="font-serif font-bold text-sm leading-snug" style={{ color: '#F5EAD8' }}>{item.name}</p>
        ...
        <div className="flex items-center gap-2 mt-2">
          <button
            ...
            style={{ border: '1px solid rgba(166,124,82,0.3)', color: '#F5EAD8' }}
            aria-label="Decrease quantity"
          ><Minus size={12} /></button>
          <span className="text-xs font-medium w-4 text-center tabular-nums" style={{ color: '#F5EAD8' }}>
            {item.quantity}
          </span>
          <button
            onClick={() => onQtyChange(item, item.quantity + 1)}
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: '#1A0A00', borderLeft: '1px solid rgba(166,124,82,0.2)' }}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
          style={{ borderBottom: '1px solid rgba(166,124,82,0.2)' }}>
          <div className="flex items-center gap-2">
            <h2 className="font-serif font-bold text-lg" style={{ color: '#F5EAD8' }}>Your Cart</h2>
            {itemCount > 0 && (
        ...
        <div className="flex-1 overflow-y-auto px-5">
          {isEmpty ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16">
              <ShoppingBag size={36} style={{ color: 'rgba(166,124,82,0.35)' }} className="mb-4" />
              <p className="font-serif font-bold text-lg mb-1" style={{ color: '#F5EAD8' }}>Your cart is empty</p>
        ...
        {!isEmpty && (
          <div className="flex-shrink-0 px-5 py-5 space-y-4"
            style={{ borderTop: '1px solid rgba(166,124,82,0.2)' }}>
            <div className="flex items-center justify-between">
              <p className="text-sm" style={{ color: '#8C7355' }}>Subtotal</p>
              <p className="font-bold text-base tabular-nums" style={{ color: '#F5EAD8' }}>
